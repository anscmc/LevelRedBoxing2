function createSearchableMap(locations = allLocations) {
  ...
  locations.forEach(function(location) {
    ...
    infoWindowContent.push(['<div class="infoWindow"><h3>' + location.name + 
                            ...
                            ', ' + location.state + ' ' + location.zip + '</p><p>' + 
                            location.phone + '</p><a href=' + location.weblink + '>View Studio</a></div>']);
  });	    
...
function filterLocations() {
  ...
  function getLatLngViaHttpRequest(address) {
    ...
    var key = 'AIzaSyDO914P3hhKOa8AoN-KRJnHgrVO6Os60AY';
    ...
    $.get( request, function( data ) {
      ...
      if (filteredLocations.length > 0) {
        ...
        searchResultsAlert.innerHTML = 'Level Red Boxing locations near ' + userAddress + ':';
      } else {
        console.log("nothing found!");
        document.getElementById('locations-near-you').innerHTML = '';
        searchResultsAlert.innerHTML = 'Sorry, no locations were found.';
      }
...
function createListOfLocations(locations) {
  ...
  locations.forEach( function(location) {
    ...
    var locationInfo = "<h4>" + location.name + "</h4><p>" + location.address + "</p>" +
                       "<p>"  + location.city + ", " + location.state + " " + location.zip + "</p><p>" + location.phone + "</p><a class='studio' href='" + location.weblink + "'>View Studio</a>";
